...
func (r *userRepository) GetByID(id int) (entity.User, error) {
	...
	if err != nil {
		...
		logger.Logger.Error("Ошибка при получении пользователя по ID",
			...
			zap.Int("id", id),
			zap.String("метод", "GetByID"))
		return entity.User{}, fmt.Errorf("ошибка при получении пользователя по ID: %w", err)
	}
...
func (r *userRepository) Create(user entity.User) (entity.User, error) {
	...
	if err != nil {
		...
		return entity.User{}, fmt.Errorf("ошибка при создании пользователя: %w", err)
	}
...
func (r *userRepository) Update(id int, user entity.User) (entity.User, error) {
	...
	if err != nil {
		...
		logger.Logger.Error("Ошибка при обновлении пользователя",
			...
			zap.Int("id", id),
			zap.String("метод", "Update"))
		return entity.User{}, fmt.Errorf("ошибка при обновлении пользователя: %w", err)
	}
